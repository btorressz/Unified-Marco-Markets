...
def test_paper_mode_can_remain_ready_while_degraded(monkeypatch):
    ...
    monkeypatch.setattr(
        readiness,
        "_database_check",
        lambda: {"status": "error", "blocking_live": True},
    )
...
def test_live_mode_blocks_on_database_schema_redis_market_and_risk(monkeypatch):
    ...
    assert set(result["blocking_checks"]) == {
        "database",
        "schema",
        "redis",
        "market_data",
...
def test_research_ingestion_degradation_is_non_blocking_in_live_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        readiness,
        "_ingestion_check",
        lambda: {"status": "degraded", "blocking_live": False},
    )
...
def test_required_live_schema_is_small_and_execution_specific():
    assert set(readiness._REQUIRED_LIVE_TABLES) == {
        "order_intents",
        "orders",
        "fills",
        "positions",
