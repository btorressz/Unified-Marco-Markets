const UI = (() => {
  ...
  function renderMultiEventReactionStatistics(data) {
    const panel = document.getElementById('geo-reaction-statistics-panel');
    if (!panel) return;
    if (window.ReactionStatisticsUI) {
      window.ReactionStatisticsUI.render(panel, data, escapeHtml);
      return;
    }
    panel.innerHTML = '<div class="empty-state-text">MULTI-EVENT STATISTICAL VALIDATION UNAVAILABLE — renderer not loaded.</div>';
  }
