...
class DerivativesRepository:
    ...
    def insert_funding(self, observation: FundingObservation, ingest_run_id=None) -> dict | None:
        ...
        if row:
            try:
                ...
                lineage={"provider_field": o.metadata.get("provider_field"),
                         "timestamp_semantics": o.timestamp_semantics},
                    metadata={"rate_kind": o.rate_kind, "interval_seconds": o.interval_seconds,
                              **o.metadata},
                )
            except Exception:
                # The observation remains valid if the generic ledger is
                # temporarily unavailable; the ingest run reports that outage.
                pass
        return row
    ...
    def latest_funding(self, venue, market, rate_kind="current", source_id=None,
                       contract_version=1):
        return self.funding_history(venue=venue, market=market, rate_kind=rate_kind,
        ...
        if row:
            try:
                ...
                quality={"aligned": True, "fresh": True,
                    ...
                    lineage=observation.get("lineage", {}), metadata=observation.get("metadata", {}))
            except Exception:
                pass
        return row
    ...
    def basis_history(self, symbol=None, venue=None, market=None, start_ts=None, end_ts=None, limit=200):
        ...
        return execute_query("""SELECT * FROM basis_observations WHERE (%s IS NULL OR symbol=%s)
            ...
            (symbol,symbol,venue,venue,market,market,start_ts,start_ts,end_ts,end_ts,limit))
