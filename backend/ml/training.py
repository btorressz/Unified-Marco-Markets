...
def train_offline(samples:list[dict[str,Any]],labels:list[int],method="logistic",**metadata):
    ...
    metrics={"folds":folds,**{key:metric(key) for key in ("accuracy","precision","recall","f1","brier")}}
    repo=repository(); repo.save_dataset(manifest)
    run_id=str(uuid.uuid4()); run={"id":run_id,"dataset_id":manifest["dataset_id"],"status":"completed","method":"logistic_regression","fold_metrics":folds,"metrics":metrics}; repo.save_training_run(run)
    ...
    result={"success":True,"method":"logistic_regression","n_samples":len(X),"dataset":manifest,"training_run":run,"model":{k:v for k,v in saved.items() if k!="artifact_blob"},"temporal_validation":folds,"cv_scores":[f["accuracy"] for f in folds],"cv_mean":metrics["accuracy"],"ts":now}
